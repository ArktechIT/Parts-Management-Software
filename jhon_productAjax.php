...
-->
	...
	if($partsQuery->num_rows > 0)
	{
		...
		while($partsQueryResult = $partsQuery->fetch_array())
		{
			...
			//TAMANG---------------------------------------------------------------------------------------------------------
			$checkBox = "<input type='checkbox' name='checkBox[]' value='".$partsQueryResult['partId']."' onclick = \" myPartsComment('".$partsQueryResult['partId']."') \">";
			//TAMANG------------------------------------------------------------------------------------------------------------------------------
			...
			$materialId = '';
			$sql = "SELECT materialId FROM system_laserparameter WHERE listId = ".$partsQueryResult['laserParameterId']." LIMIT 1";
			$queryLaserParameter = $db->query($sql);
			if($queryLaserParameter AND $queryLaserParameter->num_rows > 0)
			{
				$resultLaserParameter = $queryLaserParameter->fetch_assoc();
				$materialId = $resultLaserParameter['materialId'];
			}			
			...
			{
				$unitPriceTd = ($_SESSION['departmentId']==4) ? "<td>".$price."</td>" : "";
