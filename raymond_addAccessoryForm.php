...
<?php
echo "<center>";
echo "<form action = 'anthony_addAccessoryFormSQL.php?partId=".$partId."&patternId=".$patternId."' method = 'POST'>";
echo "<table border=1>";
	echo "<thead>";
		echo "<th colspan=2><center><span style='font-size:15px;'>".displayText('L1444')."</span></center></th>";//L1444 Add Accessory Form
	echo "</thead>";
	echo "<tbody>";
		echo "<tr>";
...
echo "<table border = 1>";
	echo "<thead class='thead'>";
		...
		echo "<th style='text-align:center;'>".displayText('L96')."</th>";//L97 Accessory Name
	...
	echo "<tbody id='ajaxResult' class='tbody'>";
		$sql = "SELECT `accessoryNumber`, `accessoryName`, `accessoryDescription`, `revisionId` FROM `cadcam_accessories` WHERE status = 0 AND cadcamAccessoryId = 0 ORDER BY accessoryNumber";
		$queryAccessory = $db->query($sql);
...
echo "<table border = 1>";
	echo "<tbody>";
		echo "<tr>";
			...
			echo "<td><input type = 'number' name = 'quantity' min = '1' style='width:330px; height:25px;'></td>";
		echo "</tr>";
		echo "<tr>";
			echo "<td style='width:120px;'><b>".displayText('L242')."</b></td>";//L242 Remarks
			echo "<td><input type = 'text' name = 'remarks' min = '1' style='width:330px; height:25px;'></td>";
		echo "</tr>";
		//~ if($_SESSION['idNumber']=='0346')
		//~ {
			echo "<tr>";
				echo "<td style='width:120px;'><b>".displayText('L59')."</b></td>";//L87 Process
				echo "<td>";
				?>
		...
		//~ }
		echo "<tr>";
			echo "<td colspan=2><center><input type ='submit' name = 'submit' value = '".displayText('L1443')."' class='anthony_submit'></center></td>";//L1443 Add Accessory
		echo "</tr>";
	echo "</tbody>";
echo "</table>";
echo "</form>";
