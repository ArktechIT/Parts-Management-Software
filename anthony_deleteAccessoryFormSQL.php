...
if($getOldSubParts->num_rows > 0)
{
	...
	$sql = "DELETE FROM engineering_subpartprocesslink WHERE partId = ".$_GET['partId']." AND patternId = ".$_GET['patternId']." AND childId = ".$oldChildId." AND identifier = 2 ";
	$queryDelete = $db->query($sql);	
}
$sql = "INSERT INTO system_partlog (partId, date, query, field, oldValue, newValue, ip, user, details) VALUES (".$_GET['partId'].", now(), 3, 12, '".$oldChildId."', '', '".$userIP."', '".$userID."', 'DELETE childId WHERE identifier = 2') ";
$insert = $db->query($sql);
